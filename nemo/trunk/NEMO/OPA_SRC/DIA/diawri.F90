...
# if defined key_iomput
   ...
   SUBROUTINE dia_wri( kt )
      ...
      REAL(wp), POINTER, DIMENSION(:,:,:) :: z3d,zhu,zhv  ! 3D workspace
      !!----------------------------------------------------------------------
      ! 
      ...
      call wrk_alloc( jpi , jpj, jpk , zhu )
      call wrk_alloc( jpi , jpj, jpk , zhv )
      !
      ...
      IF( iom_use("u_heattr") ) THEN
         DO jk = 1, jpkm1
            DO jj = 2, jpjm1
               DO ji = fs_2, fs_jpim1   ! vector opt.
                  zhu(ji,jj,jk) =  un(ji,jj,jk) * e2u(ji,jj) * fse3u(ji,jj,jk) * umask(ji,jj,jk) * ( tsn(ji,jj,jk,jp_tem) + tsn(ji+1,jj,jk,jp_tem) )
               END DO
            END DO
          END DO
         zhu(:,:,:) =  0.5 * rcp * rau0 * zhu(:,:,:) 
         CALL lbc_lnk( zhu, 'U', -1. )
         CALL iom_put( "u_heattr",  zhu )    ! heat transport in i-direction
      ENDIF
      ...
      IF( iom_use("u_salttr") ) THEN
         DO jk = 1, jpkm1
            DO jj = 2, jpjm1
               DO ji = fs_2, fs_jpim1   ! vector opt.
                  zhu(ji,jj,jk) = un(ji,jj,jk) * e2u(ji,jj) * fse3u(ji,jj,jk) * umask(ji,jj,jk) * ( tsn(ji,jj,jk,jp_sal) + tsn(ji+1,jj,jk,jp_sal) )
               END DO
            END DO
          END DO
         CALL lbc_lnk( zhu, 'U', -1. )
         CALL iom_put( "u_salttr", 0.5 * zhu )            ! salt transport in i-direction
      ENDIF
      ...
      IF( iom_use("v_heattr") ) THEN
         DO jk = 1, jpkm1
            DO jj = 2, jpjm1
               DO ji = fs_2, fs_jpim1   ! vector opt.
                  zhv(ji,jj,jk) = vn(ji,jj,jk) * e1v(ji,jj) * fse3v(ji,jj,jk) * vmask(ji,jj,jk) * ( tsn(ji,jj,jk,jp_tem) + tsn(ji,jj+1,jk,jp_tem) )
               END DO
            END DO
         END DO
         zhv(:,:,:) =  0.5 * rcp * rau0 * zhv(:,:,:)
         CALL lbc_lnk( zhv, 'V', -1. )
         CALL iom_put( "v_heattr", zhv )    !  heat transport in j-direction
      ENDIF
      ...
      IF( iom_use("v_salttr") ) THEN
         DO jk = 1, jpkm1
            DO jj = 2, jpjm1
               DO ji = fs_2, fs_jpim1   ! vector opt.
                  zhv(ji,jj,jk) = vn(ji,jj,jk) * e1v(ji,jj) * fse3v(ji,jj,jk) * vmask(ji,jj,jk) * ( tsn(ji,jj,jk,jp_sal) + tsn(ji,jj+1,jk,jp_sal) )
               END DO
            END DO
         END DO
         CALL lbc_lnk( zhv, 'V', -1. )
         CALL iom_put( "v_salttr", 0.5 * zhv )            !  salt transport in j-direction
      ENDIF
      ...
      CALL iom_put( "bn2T", rn2T)  !Brunt-Vaisala buoyancy frequency (N^2)
      CALL iom_put( "bn2S", rn2S)  !Brunt-Vaisala buoyancy frequency (N^2)

      !
      CALL wrk_dealloc( jpi , jpj      , z2d )
      CALL wrk_dealloc( jpi , jpj, jpk , z3d )
      CALL wrk_dealloc( jpi , jpj, jpk , zhu )
      CALL wrk_dealloc( jpi , jpj, jpk , zhv )
      !

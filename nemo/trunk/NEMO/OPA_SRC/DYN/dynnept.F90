MODULE dynnept
   ...
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:)    :: znepmask      !!
 ...
 CONTAINS
   ...
   INTEGER FUNCTION dynnept_alloc()
      ...
      ALLOCATE( zunep(jpi,jpj) , zvnep(jpi,jpj) ,     &
         &      znepmask(jpi,jpj), zhdivnep(jpi,jpj,jpk) , zmrotnep(jpi,jpj,jpk) , STAT=dynnept_alloc )
         !
...
!!    Perform dynamic allocation of shared module variables
      ...
      !!robinson
      CALL iom_open ( 'nepmask.nc', inum )
      CALL iom_get  ( inum, jpdom_autoglo, 'nepmask', znepmask)
      write(numout,*) 'Reading nepmask.nc'
      CALL iom_close( inum )
      CALL lbc_lnk(znepmask,'T',1.0_wp)
      !!robinson
...
!!                Apply ramp down to velocity component
            ...
            !!! robinson
            zunep(ji,jj)=zunep(ji,jj)*znepmask(ji,jj)
...
!!                Apply ramp down to velocity component
            ...
            !!! robinson
            zvnep(ji,jj)=zvnep(ji,jj)*znepmask(ji,jj)
...
!!    this routine, except where passed as arguments to other routines
   ...
   SUBROUTINE dyn_nept_vel( ksign )
      ...
      END DO
      !
      call iom_put("unept",zunep(:,:))
      call iom_put("vnept",zvnep(:,:))
